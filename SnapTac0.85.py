...
for face in ('TOP', 'BOT'):
	...
	for i, item in enumerate(frame, start=1):
		...
		# testorder/testplan device coverage check
		...
		if len_axis >= 2:
			...
			# 在图形正中间添加名称
			plt.text(center_x, center_y, name, ha='center', va='center', alpha=0.8, fontsize=fsize, color='black', rotation=rotate,
			...
			for x_coord, y_coord, name in zip(x_coords, y_coords, padname):
				...
				plt.text(x_coord, y_coord, name, ha='center', va='center', alpha=0.6, fontsize=1, color='orange', rotation=0)
	...
	for i, item in enumerate(coords_pin, start=1):
		...
		for x_coord, y_coord in zip(x_coords, y_coords):
			plt.text(x_coord, y_coord, tpname, ha='center', va='center', alpha=0.4, fontsize=1, color='black', rotation=0)
